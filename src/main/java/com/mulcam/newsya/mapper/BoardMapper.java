...
/*    @Select(" SELECT * FROM news ORDER BY DATE DESC LIMIT 4 ")
    ...
    List<BoardDto> getSpecificDateNewsByCategory(@Param("category") String category, @Param("date") Date date, int offset, int limit);

    @Select(" SELECT * FROM news WHERE DATE = #{date} ORDER BY ID DESC LIMIT LIMIT #{offset}, #{limit} ")
    List<BoardDto> getAllNewsByDate(@Param("date") Date date, int offset, int limit);

    @Select("SELECT * FROM news WHERE id >= #{id} ORDER BY id ASC LIMIT #{offset}, #{limit} ")
    List<BoardDto> getArticleListStartingFromId(@Param("id") int id, int offset, int limit);
 
    @Select("SELECT * FROM news WHERE id >= #{id} AND category = #{category} ORDER BY id ASC LIMIT #{offset}, #{limit} ")
    List<BoardDto> getArticleListStartingFromIdAndCategory(@Param("id") int id, @Param("category") String category, int offset, int limit);
